...
REPLAY_SIZE = 13000  # experience replay buffer size
BATCH_SIZE = 40  # size of minibatch
...
def init(env, env_name):
    ...
    action spaces
    """
    global replay_buffer, epsilon,iscontinuous,pieces_array
    replay_buffer = []
    ...
    if env_name == 'CartPole-v0' or env_name == 'MountainCar-v0':
        ...
        iscontinuous = 1
        lower = env.action_space.low[0]
        upper = env.action_space.high[0]
        all_p  = np.array([lower,upper],dtype=np.int32)
        #cut the action space
        pieces = pd.cut(all_p, num_cut, retbins=True)
        
        pieces_array = pieces[1]
        state_dim = env.observation_space.shape[0]
        action_dim = num_cut
...
def get_network(state_dim, action_dim, hidden_nodes=HIDDEN_NODES):
    ...
    """
    global target_input_w,target_input_b,target_hidden1_w,target_hidden1_b
    global target_q_values,target_q_selected_action
    global hidden2_w,hidden2_b,target_hidden2_w,target_hidden2_b
    global input_w,input_b,hidden1_w,hidden1_b,target_action_in
    ...
    target_action_in = tf.placeholder("float", [None, action_dim])
    ...
    input_w = tf.Variable(tf.truncated_normal([state_dim,HIDDEN_NODES], stddev=0.1))
    input_b = tf.Variable(tf.truncated_normal([HIDDEN_NODES], stddev=0.1))
    input_out = tf.matmul(state_in,input_w)+input_b
    hidden1_in = tf.nn.relu(input_out)
    hidden1_w = tf.Variable(tf.truncated_normal([HIDDEN_NODES,HIDDEN_NODES], stddev=0.1))
    hidden1_b = tf.Variable(tf.truncated_normal([HIDDEN_NODES], stddev=0.1))
    
    hidden1_out = tf.matmul(hidden1_in,hidden1_w)+hidden1_b
    hidden2_in = tf.nn.relu(hidden1_out)
    hidden2_w = tf.Variable(tf.truncated_normal([HIDDEN_NODES,action_dim], stddev=0.1))
    hidden2_b = tf.Variable(tf.truncated_normal([action_dim], stddev=0.1))
    
    q_values = tf.matmul(hidden2_in,hidden2_w)+hidden2_b
    ...
    ##That formula in slides 12  need to change to the one in page 16
    ##replace the target_in with another network
    ...
    ##target network
    target_input_w = tf.Variable(tf.truncated_normal([state_dim,HIDDEN_NODES], stddev=0.1))
    target_input_b = tf.Variable(tf.truncated_normal([HIDDEN_NODES], stddev=0.1))
    target_input_out = tf.matmul(state_in,target_input_w)+target_input_b
    target_hidden1_in = tf.nn.relu(target_input_out)
    target_hidden1_w = tf.Variable(tf.truncated_normal([HIDDEN_NODES,HIDDEN_NODES], stddev=0.1))
    target_hidden1_b = tf.Variable(tf.truncated_normal([HIDDEN_NODES], stddev=0.1))
    target_hidden1_out = tf.matmul(target_hidden1_in,target_hidden1_w)+target_hidden1_b
    target_hidden2_in = tf.nn.relu(target_hidden1_out)
    target_hidden2_w = tf.Variable(tf.truncated_normal([HIDDEN_NODES,action_dim], stddev=0.1))
    target_hidden2_b = tf.Variable(tf.truncated_normal([action_dim], stddev=0.1))
    target_q_values = tf.matmul(target_hidden2_in,target_hidden2_w)+target_hidden2_b
    target_q_selected_action = \
        tf.reduce_sum(tf.multiply(target_q_values, target_action_in), reduction_indices=1)
...
def update_replay_buffer(replay_buffer, state, action, reward, next_state, done,
    ...
    one_hot_action = np.zeros(action_dim,dtype=np.int32)
...
def get_train_batch(q_values, state_in, minibatch):
    ...
    #produce a one_hot encoded max action matrix
    Q_value_batch_zeros = np.zeros_like(Q_value_batch)
    Q_value_batch_zeros[np.arange(len(Q_value_batch)), Q_value_batch.argmax(1)] = 1
    DQL_target = target_q_selected_action.eval(feed_dict={
        state_in: next_state_batch,
        target_action_in: Q_value_batch_zeros
    ...
    for i in range(0, BATCH_SIZE):
        ...
        if sample_is_done:
            ...
            target_val = GAMMA * DQL_target[i]+reward_batch[i]
...
def qtrain(env, state_dim, action_dim,
    ...
    session.run(tf.assign(target_input_w,input_w))
    session.run(tf.assign(target_input_b,input_b))
    session.run(tf.assign(target_hidden1_w,hidden1_w))
    session.run(tf.assign(target_hidden1_b,hidden1_b))
    session.run(tf.assign(target_hidden2_w,hidden2_w))
    session.run(tf.assign(target_hidden2_b,hidden2_b))
